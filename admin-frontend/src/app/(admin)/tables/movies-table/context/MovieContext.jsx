...
export const MovieProvider = ({ children }) => {
  ...
  const buildApiUrl = () => {
    const params = new URLSearchParams({
      ...
      sortBy: state.sortBy || 'movieReleaseDate', // Trường sắp xếp
      sortDirection: state.sortDirection || 'DESC', // Hướng sắp xếp
    });
  ...
  useEffect(() => {
    fetchMovies();
    fetchRatings();
  }, [state.currentPage, state.query, state.filters, state.sortBy, state.sortDirection]);
